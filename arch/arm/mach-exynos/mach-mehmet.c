/* linux/arch/arm/mach-exynos/mach-mehmet.c
 *
...
#define MEHMET_BOOT_NORMAL			1
#define MEHMET_BOOT_RECOVERY		2
#define MEHMET_BOOT_HOTKEY_UPDATE	3
#define MEHMET_BOOT_FACTORYTEST_L	4
#define MEHMET_BOOT_FACTORYTEST_H	5
#define MEHMET_BOOT_FASTBOOT      6
...
MEHMET_HW_VERSION g_mehmet_hw_version = MEHMET_HW_UNKNOWN;

void mehmet_check_hw_version( void )
{
	static int ADC0_HW = 0; // GPL2_2, GPIO_HW_VERSION_0
	static int ADC1_HW = 0; // GPL2_1, GPIO_HW_VERSION_1
	static int ADC2_HW = 0; // GPL2_0, GPIO_HW_VERSION_2
	static int ADC3_HW = 0; // GPL2_3, GPIO_HW_VERSION_3
	...
	char *str_version[] = {
		...
		"MEHMET_HW_MP", // 3
		"MEHMET_HW_MP2", // 4
		"MEHMET_HW_MP3", // 5, LG touch
		"MEHMET_HW_MP4", // 6, HS touch
		"MEHMET_HW_VE1", // 7, HS touch
		...
		"MEHMET_HW_VD4", // 13
		"MEHMET_HW_VE3", // 14
		"MEHMET_HW_VE2", // 15, LG touch
		"MEHMET_HW_UNKNOWN" // 16
	};

	/* Set GPL2[0],[1],[2],[3] Control Register to input */
	...
	s3c_gpio_cfgpin(GPIO_HW_VERSION_3, S3C_GPIO_SFN(0x0));
	...
	s3c_gpio_setpull(GPIO_HW_VERSION_3, S3C_GPIO_PULL_NONE);
	...
	ADC3_HW = gpio_get_value(GPIO_HW_VERSION_3) << 3; //bit3
	printk("ADC3_HW[0x%01X]\n", ADC3_HW);

	g_mehmet_hw_version = ADC3_HW | ADC2_HW | ADC1_HW | ADC0_HW;
	printk("g_mehmet_hw_version[0x%01X]\n", g_mehmet_hw_version);

	printk("MEHMET HW_VERSION: [%s]\n", str_version[g_mehmet_hw_version]);
}

MEHMET_HW_VERSION mehmet_get_hw_version( void )
{
	static int ADC0_HW = 0; // GPL2_2, GPIO_HW_VERSION_0
	static int ADC1_HW = 0; // GPL2_1, GPIO_HW_VERSION_1
	static int ADC2_HW = 0; // GPL2_0, GPIO_HW_VERSION_2
	...
	return g_mehmet_hw_version = ADC3_HW | ADC2_HW | ADC1_HW | ADC0_HW;
}
EXPORT_SYMBOL(mehmet_get_hw_version);
#endif /* CONFIG_MACH_MEHMET */

/* Following are default values for UCON, ULCON and UFCON UART registers */
#define MEHMET_UCON_DEFAULT	(S3C2410_UCON_TXILEVEL |	\
...
#define MEHMET_ULCON_DEFAULT	S3C2410_LCON_CS8

#define MEHMET_UFCON_DEFAULT	(S3C2410_UFCON_FIFOMODE |	\
				 S5PV210_UFCON_TXTRIG4 |	\
				 S5PV210_UFCON_RXTRIG4)

static struct s3c2410_uartcfg mehmet_uartcfgs[] __initdata = {
	[0] = {
		.hwport		= 0,
		.flags		= 0,
		.ucon		= MEHMET_UCON_DEFAULT,
		.ulcon		= MEHMET_ULCON_DEFAULT,
		.ufcon		= MEHMET_UFCON_DEFAULT,
	},
	[1] = {
		.hwport		= 1,
		.flags		= 0,
...
static struct mt9m113_platform_data mt9m113_plat = {
	.default_width = MEHMET_PREVIEW_MIN_W,//480,
	.default_height = MEHMET_PREVIEW_MIN_H,
	.max_width = MEHMET_PREVIEW_MAX_W,//960,
	.max_height =MEHMET_PREVIEW_MAX_H,
...
static struct s3c_platform_camera mt9m113 = {
	...
	.clk_rate	= 24000000,
	.line_length	= MEHMET_PREVIEW_MAX_W,
	.width		= MEHMET_PREVIEW_MAX_W,
	.height		= MEHMET_PREVIEW_MAX_H,
	.window		= {
		.left	= 0,
		.top	= 0,
		.width	= MEHMET_PREVIEW_MAX_W,
...
static struct platform_device mehmet_charger = {
...
static void mehmet_power_off(void)
{
...
static void mehmet_pm_restart(char mode, const char *cmd)
{
  if (cmd!=0){
	  printk("%s : %s\n",__func__,cmd);
    if(strncmp(cmd,"recovery",7)==0){
      writel(MEHMET_BOOT_RECOVERY, S5P_INFORM5);
    }
    else if(strncmp(cmd,"hotkey_update",13)==0){
      writel(MEHMET_BOOT_HOTKEY_UPDATE, S5P_INFORM5);
    }
    else if(strncmp(cmd,"FACTORYTEST_L",13)==0){
      writel(MEHMET_BOOT_FACTORYTEST_L, S5P_INFORM5);
    }
    else if(strncmp(cmd,"FACTORYTEST_H",13)==0){
      writel(MEHMET_BOOT_FACTORYTEST_H, S5P_INFORM5);
    }
    else if(strncmp(cmd,"fastboot",8)==0){
      writel(MEHMET_BOOT_FASTBOOT, S5P_INFORM5);
    }
    //unknown reboot command, anyway set register to normal boot
    else
      writel(MEHMET_BOOT_NORMAL, S5P_INFORM5);
  }
  else
    writel(MEHMET_BOOT_NORMAL, S5P_INFORM5);
...
static struct s3c_sdhci_platdata mehmet_hsmmc0_pdata __initdata = {
...
static struct s3c_sdhci_platdata mehmet_hsmmc1_pdata __initdata = {
	.cd_type		= S3C_SDHCI_CD_INTERNAL,
	.clk_type		= S3C_SDHCI_CLK_DIV_EXTERNAL,
};
#endif
...
static struct s3c_sdhci_platdata mehmet_hsmmc2_pdata __initdata = {
	.cd_type		= S3C_SDHCI_CD_INTERNAL,
	.clk_type		= S3C_SDHCI_CLK_DIV_EXTERNAL,
...
static struct s3c_sdhci_platdata mehmet_hsmmc2_pdata_MP __initdata = {
...
#ifdef CONFIG_S3C_DEV_HSMMC3
static struct s3c_sdhci_platdata mehmet_hsmmc3_pdata __initdata = {
...
static struct s5p_ehci_platdata mehmet_ehci_pdata;

static void __init mehmet_ehci_init(void)
{
	struct s5p_ehci_platdata *pdata = &mehmet_ehci_pdata;
...
static struct s5p_ohci_platdata mehmet_ohci_pdata;

static void __init mehmet_ohci_init(void)
{
	struct s5p_ohci_platdata *pdata = &mehmet_ohci_pdata;
...
static struct s5p_usbgadget_platdata mehmet_usbgadget_pdata;

static void __init mehmet_usbgadget_init(void)
{
	struct s5p_usbgadget_platdata *pdata = &mehmet_usbgadget_pdata;
...
struct sec_jack_platform_data mehmet_jack_pdata = {
...
static struct platform_device mehmet_device_jack = {
  .name = "jack_mgr",
  .id			  = 1, /* will be used also for gpio_event id */
  .dev.platform_data	= &mehmet_jack_pdata,
};
...
static struct gpio_keys_button mehmet_button[] = {
	{
		.code               = KEY_POWER,
        .gpio               = MEHMET_POWER_KEY,
		.active_low         = 1,
...
static struct gpio_keys_platform_data mehmet_gpiokeys_platform_data = {
	.buttons		= mehmet_button,
	.nbuttons		= ARRAY_SIZE(mehmet_button),
};

static struct platform_device mehmet_gpio_keys = {
	.name	= "gpio-keys",
	.dev	= {
		.platform_data = &mehmet_gpiokeys_platform_data,
	},
};

static void mehmet_gpio_key_cfg(void)
{
	s3c_gpio_cfgpin(MEHMET_POWER_KEY, S3C_GPIO_SFN(0x0));
	s3c_gpio_setpull(MEHMET_POWER_KEY, S3C_GPIO_PULL_UP);
	s3c_gpio_cfgpin(MEHMET_VOLUM_DOWN, S3C_GPIO_SFN(0x0));
	s3c_gpio_setpull(MEHMET_VOLUM_DOWN, S3C_GPIO_PULL_UP);
	s3c_gpio_cfgpin(MEHMET_VOLUM_UP, S3C_GPIO_SFN(0x0));
	s3c_gpio_setpull(MEHMET_VOLUM_UP, S3C_GPIO_PULL_UP);
}

static void mehmet_gpio_pmint_cfg(void)
{
	s3c_gpio_cfgpin(GPIO_PMIC_IRQ, S3C_GPIO_SFN(0xF));
	s3c_gpio_setpull(GPIO_PMIC_IRQ, S3C_GPIO_PULL_UP);
}

static void mehmet_gpio_i2c_cfg(void)
{
	//GSENSOR
...
static struct s5p_usbswitch_platdata mehmet_usbswitch_pdata;

static void __init mehmet_usbswitch_init(void)
{
...
#ifdef CONFIG_LEDS_S3C24XX
static struct s3c24xx_led_platdata smt_led_pdata = {
	.name		= "smt_led",
	.gpio		= EXYNOS4212_GPM0(1),
};

static struct platform_device smt_led = {
	.name		= "s3c24xx_led",
	.id		= 1,
	.dev		= {
		.platform_data	= &smt_led_pdata,
	},
};
#endif

static struct platform_device *mehmet_devices[] __initdata = {
#ifdef CONFIG_ANDROID_PMEM
	&pmem_device,
	&pmem_gpu1_device,
#endif
...
#error CONFIG_FB_S5P is defined. Select CONFIG_FB_S3C, instead
	...
	static const char map[] __initconst =
...
#endif
		"s3c-fimc.0=fimc0;s3c-fimc.1=fimc1;s3c-fimc.3=fimc3;"
		"exynos4210-fimc.0=fimc0;exynos4210-fimc.1=fimc1;exynos4210-fimc.3=fimc3;"
		...
		"s5p-fimg2d=fimg2d;"
#endif
		"ion-exynos=ion,fimd,fimc0,fimc1,fimc3,fw,b1,b2;"
#ifdef CONFIG_EXYNOS_CONTENT_PATH_PROTECTION
...
/* LCD Backlight data */
static struct samsung_bl_gpio_info mehmet_bl_gpio_info = {
	.no = BACKLIGHT_PWM_GPIO,
	.func = S3C_GPIO_SFN(2),
};

static struct platform_pwm_backlight_data mehmet_bl_data = {
	.pwm_id = 1,
	.max_brightness = 255,
	.dft_brightness = 255,
	.lth_brightness = 17,
...
static void __init mehmet_map_io(void)
{
	clk_xusbxti.rate = 24000000;
	s5p_init_io(NULL, 0, S5P_VA_CHIPID);
	s3c24xx_init_clocks(24000000);
	s3c24xx_init_uarts(mehmet_uartcfgs, ARRAY_SIZE(mehmet_uartcfgs));
...
static void __init mehmet_machine_init(void)
{
	arm_pm_restart = mehmet_pm_restart;
	pm_power_off = mehmet_power_off;
...
#endif
#ifdef CONFIG_USBHUB_USB3503
	if(mehmet_get_hw_version() >= MEHMET_HW_MVT)
		usb3503_init();
#endif
...
#endif
#ifdef CONFIG_USB_GADGET
	mehmet_usbgadget_init();
#endif
#ifdef CONFIG_USB_EXYNOS_SWITCH
	mehmet_usbswitch_init();
#endif
#ifdef CONFIG_ISL29023
	s3c_gpio_setpull(GPIO_LSENSOR_INT, S3C_GPIO_PULL_UP);
#endif
#ifdef CONFIG_JACK_MGR
	struct sec_jack_platform_data *jack_mgr_pdata = mehmet_device_jack.dev.platform_data;
	int is_jack_mgr_mp2 = 0;
	if(mehmet_get_hw_version() > MEHMET_HW_MVT){
		s3c_gpio_setpull(GPIO_REMOTE_KEY_INT, S3C_GPIO_PULL_DOWN);
		if(mehmet_get_hw_version() > MEHMET_HW_MP)
			is_jack_mgr_mp2 = 1;
	}else{
	...
	samsung_bl_set(&mehmet_bl_gpio_info, &mehmet_bl_data);
...
#endif
#ifdef CONFIG_S3C_DEV_HSMMC
	s3c_sdhci0_set_platdata(&mehmet_hsmmc0_pdata);
#endif
#ifdef CONFIG_S3C_DEV_HSMMC1
	s3c_sdhci1_set_platdata(&mehmet_hsmmc1_pdata);
#endif
#ifdef CONFIG_S3C_DEV_HSMMC2
	if(mehmet_get_hw_version() >= MEHMET_HW_MP)
		s3c_sdhci2_set_platdata(&mehmet_hsmmc2_pdata_MP);
	else
		s3c_sdhci2_set_platdata(&mehmet_hsmmc2_pdata);
#endif
#ifdef CONFIG_S3C_DEV_HSMMC3
	s3c_sdhci3_set_platdata(&mehmet_hsmmc3_pdata);
#endif
#ifdef CONFIG_EXYNOS4_DEV_MSHC
	...
	platform_add_devices(mehmet_devices, ARRAY_SIZE(mehmet_devices));
